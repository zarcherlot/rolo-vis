...
/**
 * Merge a producer bundle into the local matrix without granting baseline authority.
 * Only an explicit VERIFIED producer item can update a scenario to VERIFIED; missing,
 * pending, and blocked items remain unresolved and always require human review.
 */
export function importDeviceHardeningEvidence(
  rawMatrix: unknown,
  rawBundle: unknown,
): DeviceHardeningEvidenceImport {
  ...
  const scenarios: DeviceHardeningScenario[] = matrix.scenarios.map((scenario) => {
    ...
    if (item.status === "VERIFIED" && item.evidence) {
      return { ...scenario, status: "VERIFIED", evidence: item.evidence };
    }
    return {
      ...scenario,
      status: item.status,
