...
- paired rolo harness 已验证并停止；由于尚无 staging/真机控制面证据，E24、R1、R2 与 Artifact Analysis 仍保持 candidate，不提前提升 baseline。外部收口流程见 `../../ROLO_EXTERNAL_CLOSURE_RUNBOOK.md`。
...
5. Target Readiness、Approval/Gate/Recovery、Artifact Analysis 已接入严格解析器、client、feature-gated 只读 UI 与 live gate 脚本；待 staging/生产证据后再提升 baseline。

## 开发顺序

### P0：完成 Job/R1/R2 真实控制面验收

前置已满足：rolo main `8441e222` 已发布 Job、R1、R2、Artifact Analysis producer；rolo-vis consumer 已合入 `main`。
...
当前状态：deterministic harness 已通过；下一步是 staging/生产控制面证据，完成后再将三个基础 contract 提升为 baseline。

### P1：激活 R1 Target Readiness

前置已满足：rolo main `8441e222`（PR #47）发布 `rolo-target-readiness-summary/v1` 与 `workbench.target-readiness/v1`。
...
当前状态：consumer 已合入；harness gate 已通过，等待 staging/生产证据晋级 baseline。

### P1：激活 R2 Approval/Gate/Recovery（只读）

前置已满足：rolo main `8441e222`（PR #48）发布 `rolo-approval-gate-summary/v1`，并由 `/health` 宣布对应 feature gate。
...
完成标准：真实 API 优先、API 不可用时明确 demo；没有 silent fallback、任意路径读取或权限边界放宽。当前状态：consumer/parser/plugin/UI 已合入；target/job live gate 均已通过 rolo `main@8441e222` harness，等待 staging/生产证据。
...
1. **P0：staging/production live evidence 收口**。由 rolo 团队提供可访问控制面、固定
   revision、`/health.api_features` 和脱敏响应证据；rolo-vis 重跑四组 gate（Job、R1/R2、
   Artifact Analysis），再把对应 candidate contract 晋级 baseline。
2. **P1：E26 设备侧加固验证**。按设备矩阵补齐 Linux/Windows、断网、重启、host-key rotation、
   离线安装与 rollback 证据；只接入 Evidence ledger，不开放浏览器执行入口。
3. **P1：live UI 验收与可访问性收口**。覆盖 Analysis 的 fresh/stale/unknown/404/409、旧 rolo
   feature 缺失、跨 target/job 切换、键盘焦点、窄屏和 reduced-motion；保持 demo 仅在用户显式
   选择后可见。
4. **P2：Artifact Analysis 读模型扩展**。在 producer contract 不变的前提下增加 partial/stale
