...
import { parseDeviceHardeningMatrix } from "../src/contracts/deviceHardening.ts";

const matrix = parseDeviceHardeningMatrix(JSON.parse(await readFile(new URL("../tests/fixtures/device-hardening-matrix.json", import.meta.url), "utf8")));
...
for (const scenario of matrix.scenarios) {
  ...
  assert.ok(["AUTOMATED_CHECKED", "PENDING_EXTERNAL", "BLOCKED", "VERIFIED"].includes(scenario.status));
